...
export default function PurchaseLink({ platformKey, url, className = '' }: PurchaseLinkProps) {
  const { displayText, platformClass, IconComponent } = getPlatformInfo(platformKey);
  ...
  return (
    <a
      ...
      className={`buy-button ${platformClass} ${className}`}
    >
      {IconComponent}
      {displayText}
    </a>
...
function getPlatformInfo(platformKey: string) {
  ...
  let displayText = 'Купить';
  let platformClass = '';
  let IconComponent = null;

  if (platformKey.startsWith('kk_')) {
    displayText = 'Купить на Kupikod';
  } else if (platformKey.startsWith('pm_')) {
    displayText = 'Купить на Plati.Market';
  }

  // Определяем платформу и соответствующие стили/иконки
  if (platformKey.includes('_xbox_')) {
    platformClass = 'xbox-platform';
    IconComponent = <FaXbox className="w-4 h-4" />;
  } else if (platformKey.includes('_steam_')) {
    platformClass = 'steam-platform';
    IconComponent = <FaSteamSymbol className="w-4 h-4" />;
  } else if (platformKey.includes('_rockstar_')) {
    platformClass = 'rockstar-platform';
    IconComponent = <RockstarIcon />;
  }

  return { displayText, platformClass, IconComponent };
}